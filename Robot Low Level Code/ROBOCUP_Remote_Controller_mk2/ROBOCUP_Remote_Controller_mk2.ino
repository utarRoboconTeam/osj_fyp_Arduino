...
ROBOCUP robot controller using ESP32-S3 Microcontroller
...
#include "bitmaps.h"  // for the OLED intro splashscreen
...
uint8_t broadcastAddress1[] = { 0xE4, 0x65, 0xB8, 0x12, 0x10, 0x68 };  // Main controller MAC address
uint8_t broadcastAddress2[] = { 0x7C, 0x87, 0xCE, 0x30, 0x1B, 0x48 };  // Base controller MAC address
uint8_t broadcastAddress3[] = { 0xE4, 0x65, 0xB8, 0x11, 0xBC, 0x94 };  // Arm controller MAC address
...
// Joystick
float x, y, z, zPower, d;   // command velocities from joysticks
int joystickx, joysticky, joystickz, joystickd; // joystick readings
...
// Potentiometers
// For the robot arm gripper
float rollArm, pitchArm, gripArm;

// left outermost potentiometer averaging filter
int POT1readings[MaxReadings];
int POT1readIndex = 0;
int POT1total = 0;
int POT1_Pos = 0;

// left innermost potentiometer averaging filter
int POT2readings[MaxReadings];
int POT2readIndex = 0;
int POT2total = 0;
int POT2_Pos = 0;

// right innermost potentiometer averaging filter
int POT3readings[MaxReadings];
int POT3readIndex = 0;
int POT3total = 0;
int POT3_Pos = 0;

// right outermost potentiometer averaging filter
...
// Buttons
// For mode changing
int enableReading, prevEnableReading; // auto or manual mode changing
int baseOrArmReading, prevBaseOrArmReading; // base control or arm control changing
...
// Robot arm telemetry data
long int xArmNow = 0, yArmNow = 0, zArmNow = 0;
unsigned short int rollArmNow = 0, pitchArmNow = 0, gripArmNow = 0, forceReading = 0;


// Flags to indicate certain operations
bool autoOrManual = false; // true to make it auto only
bool updateDisplay = true;  // true to update the display
bool baseOrArm = false; // true to make it control the robot arm only
bool homeAllAxes = false;
bool odomClearing = false;
...
unsigned long prevBaseOrArmMillis = 0;
...
void OnDataRecv(const uint8_t* mac, const uint8_t* incomingData, int len) {
  ...
  else if (tele.deviceID == ARM_TELEM){
    xArmNow = tele.xArmNow;
    yArmNow = tele.yArmNow;
    zArmNow = tele.zArmNow;
    rollArmNow = tele.rollArmNow;
    pitchArmNow = tele.pitchArmNow;
    gripArmNow = tele.gripArmNow;
    forceReading = tele.forceReading;
  }
}
...
void normalDisplay(){
  ...
  if (!baseOrArm){
    if (!odomClearing){
      ...
      display.drawCircle(110 - (int)(zPos * 15), 20, 1, SH110X_WHITE);
    }
    else {
      display.setTextColor(SH110X_WHITE);
  ...
  // controlling robot arm
    ...
    display.drawLine(10, 10, 10, 64, SH110X_WHITE);
    display.drawLine(0, 54, 128, 54, SH110X_WHITE);

    display.drawLine(64, 60, 64 + map(POT1_Pos, 0, 255, 0, 60), 60, SH110X_WHITE);
    display.drawLine(100, 15, 100 + map(POT2_Pos, 0, 255, 0, 20), 15, SH110X_WHITE);
    display.drawLine(100, 25, 100 + map(POT3_Pos, 0, 255, 0, 20), 25, SH110X_WHITE);
    display.drawLine(100, 35, 100 + map(POT4_Pos, 0, 255, 0, 20), 35, SH110X_WHITE);
    ...
    if (homeAllAxes){
      display.print("TRIGGERED HOMING");
    }

    display.drawCircle(5, 32 - (int)(yArm * 15), 1, SH110X_WHITE);
    display.drawCircle(34 + (int)(xArm * 15), 60, 1, SH110X_WHITE);
    display.drawCircle(0, 64, (255 + (int)(zArm)) / 30, SH110X_WHITE);
...
// Reflect the transmission status of the ESP-NOW from remote to the robot using addressable LED
...
void loop() {
  ...
  if (pidTune){
    ...
    pidChngModeNextPage = digitalRead(L2) == HIGH ? true : false;
    ...
    if (currMillis - prevESPNOWMillis >= ESPNOW_TIMEFRAME){
      esp_err_t result1 = esp_now_send(broadcastAddress1, (uint8_t *)&tele, sizeof(tele));
      esp_err_t result2 = esp_now_send(broadcastAddress2, (uint8_t *)&tele, sizeof(tele));
  ...
  // ========================================================
    ...
    enableReading = digitalRead(B3);
    if (enableReading == 1 && prevEnableReading == 0 && currMillis - prevEnableMillis >= debounce) {
    ...
    baseOrArmReading = digitalRead(B2);
    if (baseOrArmReading == 1 && prevBaseOrArmReading == 0 && currMillis - prevBaseOrArmMillis >= debounce) {
      baseOrArm = !baseOrArm;
      prevBaseOrArmMillis = currMillis;
    }
    prevBaseOrArmReading = baseOrArmReading;


    if (!baseOrArm){
      odomClearing = digitalRead(B1);
      xPos = xBuf;
      yPos = yBuf;        
      zPos = zBuf;
      tele.deviceID = BASE_TELEOP;
      ...
      tele.yPos = yPos;
      tele.wPos = zPos;    
      tele.clearOdom = odomClearing;
    }
    else {
      homeAllAxes = digitalRead(B1);
      xArm = xBuf;
      yArm = yBuf;
      zArm = dBuf;
      rollArm = POT2_Pos;
      pitchArm = POT3_Pos;
      gripArm = POT4_Pos;
      tele.deviceID = ARM_TELEOP;
      tele.xArm = xArm;
      tele.yArm = yArm;
      tele.zArm = -zArm;
      tele.rollArm = rollArm;
      tele.pitchArm = pitchArm;
      tele.gripArm = gripArm;
      tele.homeAllAxes = homeAllAxes;
    }
    ...
    // send the data
    if (currMillis - prevESPNOWMillis >= ESPNOW_TIMEFRAME){
      esp_err_t toBase = esp_now_send(broadcastAddress1, (uint8_t *)&tele, sizeof(tele));
      esp_err_t toArm = esp_now_send(broadcastAddress3, (uint8_t *)&tele, sizeof(tele));
      prevESPNOWMillis = currMillis;
    }
